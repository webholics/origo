...
author:   
version:  2008-09-12 (Tweak: loop count in replacePlaceholders
                      Addition: "replace()" in getPropertyValue)
*/
...
class ARC2_SPARQLScriptProcessor extends ARC2_Class {
  ...
  function getPropertyValue($obj, $path) {
    ...
    if (preg_match('/^replace\([\'\"](.*)[\'\"],\s*[\'\"](.*)[\'\"]\)$/s', $path, $m)) {
      return @preg_replace($m[1], $m[2], $val);
    }
    /* struct */
    if (is_array($val)) {
